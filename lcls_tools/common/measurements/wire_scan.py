...
class WireBeamProfileMeasurement(Measurement):
    ...
    def create_device_dictionary(self):
        ...
        for d_str in self.my_wire.metadata.detectors:
            ...
            creator = next((
                f for prefix, f in create_by_prefix.items()
                if name.startswith(prefix)), None

                )
            if creator is None:
                self.logger.warning("Unknown detector type '%s'"
                                    "in '%s'. Skipping.", name, d_str)
    ...
    def get_profile_range_indices(self):
        ...
        for p in ap:
            ...
            if position_data.min() == position_data.max():  # No change in data
                msg = "Data did not collect properly in BSA buffer. " \
                      "Exiting scan."
                self.logger.error(msg)
                ...
                msg = f"Scan did not reach expected {p} profile range.  " \
                       "Exiting scan."
                self.logger.error(msg)
                raise RuntimeError(msg)
